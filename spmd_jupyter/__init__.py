...
try:
    __version__ = version("spmd-jupyter")
...
def init_process_group(*args: Any, timeout: str | int | float = "24h", **kwargs: Any) -> Any:
    ...
    This is a lazy import so importing :mod:`spmd_jupyter` does not initialize
    PyTorch or CUDA. Explicit ``datetime.timedelta`` values can be passed via
    ``timeout`` unchanged; strings such as ``"24h"`` are parsed by the runtime
    helper.
    """
